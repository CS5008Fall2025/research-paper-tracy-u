...
#define PRINT_DEBUG(...) if (DEBUG) printf("(DEBUG): " __VA_ARGS__); // This is a macro that will print if debug is true.
#define PRINT_VERBOSE(...) if (VERBOSE) printf(__VA_ARGS__); // This is a macro that will print if verbose is true.
...
*/
void add_test(TestingSet* test_set, char* name, char* group_tag, bool (*func)()) {
...
int run_tests(TestingSet *test_set, int start_index, int end_index) {
    ...
    int test_num = 0;
    Test *test = test_set->head;
    while (test != NULL) { // loop through all the tests, until start index
        ...
        test_num++;
        if(test_num > end_index) { // early exit
            break;
        }
        test = test->next;
    }
...
int run_tests_group(TestingSet *test_set, char *group_tag) {
    ...
    Test *test = test_set->head;
    while (test != NULL) { // loop through all the tests, until start index
        if (strcasecmp(test->group_tag, group_tag) == 0)
        {
            if (run_test(test)) {
                num_passed++;
            }
